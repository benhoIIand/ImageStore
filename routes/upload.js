...
var downloadImage = function(uri, cb) {
    request.head(uri, function(err, res) {
        ...
        var filename = path.normalize(__dirname + '/../store/tmp/image.' + (+new Date()) + '.' + Math.floor(Math.random() * 100000000000000) + path.extname(uri)).split('?')[0];
